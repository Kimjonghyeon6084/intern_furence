...
//        }
        for (Object arg : args) {
            if (arg instanceof MultipartFile file) {
                String fileName = file.getOriginalFilename();
                if (fileName == null || Arrays.stream(allowedExtensions)
                        .noneMatch(ext-> fileName.toLowerCase().endsWith(ext.toLowerCase()))) {
                    throw new CustomException(
                            ErrorCode.INVALID_FILE_EXTENSION.getMessage());
                }
            }
        }
